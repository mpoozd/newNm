...
class Sregister extends CI_Controller {
   ...
   /*********** File Type Check *****/
	...
	function save_jobseeker(){
			...
			$ser['serial_no']=$last_id;
		 $this->db->update('jobseekers',$ser);
	    $this->db->where('jos_id',$last_id);
		
			$tag=get_info('s_reg_step2','hide_tags');
			if (!empty($tag)){
			$array = explode(',', $tag); //split string into array seperated by ', '
foreach($array as $value) //loop over values
{
  
	$sty['jos_id']=$last_id;
	$sty['tag_id']=$value;
	$this->General_model->insert_info('jobseeker_tags',$sty);
}}
			
			
			
			
			$data_edu['jos_id']=$last_id;
			$data_exp['jos_id']=$last_id;
			$data_skill['jos_id']=$last_id;
			
			
			
	  
	   
	   $this->General_model->update_where(array('jos_id'=>0),'jobseeker_experience',$data_exp);
	   $this->General_model->update_where(array('jos_id'=>0),'jobseeker_skills',$data_exp);
	   $this->General_model->update_where(array('jos_id'=>0),'jobseeker_eduation',$data_exp);
			$this->register_mail($last_id,$data_c['activation_link']);
			 
		   $this->General_model->update_where(array('jos_id'=>0),'jobseeker_experience',$data_exp);
		$this->session->set_flashdata('msg','Thank you message for student , we will active your accout as soon as possiable');
